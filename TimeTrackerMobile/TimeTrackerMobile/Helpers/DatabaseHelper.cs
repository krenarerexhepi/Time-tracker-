...
namespace TimeTrackerMobile
{
	public class DatabaseHelper
	{
		...
		{
			var connection = new SqliteConnection (connectionDb);
			string sql = "UPDATE Companies SET UserId = @UserId, CompanyName = @CompanyName WHERE CompanyId = @CompanyId";
			var cmd = connection.CreateCommand ();
			cmd.CommandText = sql;
		...
		public Company GetCompanyForUser (int userId)
		{
			...
			using (var connection = new SqliteConnection (connectionDb)) {
				...
				while (id1.Read ()) {
					...
					com.CompanyId = Convert.ToInt32 (id1 ["CompanyId"]);
		...
		public void deleteConnectionByCompanyId (int companyId)
		{
			var sql = "DELETE FROM Connections WHERE CompanyId='" + companyId + "'"; 
		...
		public long RegisterEvidence (DateTime checkInTime, DateTime checkOutTime, int connectionId, int companyId, int userId)
		{
			try {
				using (var connection = new SqliteConnection (connectionDb)) {
					var cmd = connection.CreateCommand ();
					...
					/*	cmd.Parameters.AddWithValue ("CheckInTime", Convert.ToDateTime (checkInTime));
					if (checkOutTime == "") {
						cmd.Parameters.AddWithValue ("CheckOutTime", "00/00/0000 00:00:00");
					} else {
						cmd.Parameters.AddWithValue ("CheckOutTime", Convert.ToDateTime (checkOutTime));
					}*/
		...
		public  long UpdateEvidence (int evidenceId, DateTime checkInTime, DateTime checkOutTime, int connectionId, int companyId, int userId)
		{
			var connection = new SqliteConnection (connectionDb);
		...
		public List<Evidence> GetEvidenceUserCompanyList (int userId, int companyId)
		{
			...
			using (var connection = new SqliteConnection (connectionDb)) {
				...
				while (id1.Read ()) {
					...
					con.CheckInTime = id1.GetDateTime (1);
					con.CheckOutTime = id1.GetDateTime (2);
					con.ConnectionId = Convert.ToInt32 (id1 ["ConnectionId"]);
					con.CompanyId = Convert.ToInt32 (id1 ["CompanyId"]);
					con.UserId = Convert.ToInt32 (id1 ["UserId"]);
					allEvide.Add (con);
		...
		public List<Evidence> GetAllEvidenceList ()
		{
			List<Evidence> lista = new List<Evidence> ();
			using (var connection = new SqliteConnection (connectionDb)) {
				var cmd = connection.CreateCommand ();
				...
				while (id1.Read ()) {
					Evidence con = new Evidence ();
					con.ConnectionId = Convert.ToInt32 (id1 ["EvidenceId"]);
					con.CheckInTime = id1.GetDateTime (1);
					con.CheckOutTime = id1.GetDateTime (2);
					lista.Add (con);
				}
