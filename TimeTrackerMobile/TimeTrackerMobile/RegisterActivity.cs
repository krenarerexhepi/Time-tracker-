...
{
	[Activity (Label = "Register ", Theme = "@android:style/Theme.Holo.Light")]			
					...
					try {
						if (logIn.Equals ("LogIn")) {
							try {
								...
								int companyId = companyForuser.CompanyId;
								data.UpdateCompany (companyId, lonInUserDataUserId, company.Text);

								Toast.MakeText (BaseContext, "Te dhenat u ndryshuan me sukses! ", ToastLength.Long).Show ();
