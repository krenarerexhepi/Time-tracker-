...
{
	[Activity (Label = "ConnectionsActivity")]			
	public class ConnectionsActivity : Activity
	{
		...
		protected override void OnCreate (Bundle bundle)
		{
			...
			//refresh.SetOnClickListener += refresh_SetOnClickListener;
			...
			lsWifil.ItemClick += delegate(object sender, AdapterView.ItemClickEventArgs e) {   
				...
				alert.SetButton ("Yes", (s, ev) => {
					if (!CheckSSID (SSID)) {
						...
						Toast.MakeText (ApplicationContext, "Ky wifi ekziston zgjedh nje tjeter !!", ToastLength.Long).Show ();
					}
				});
		...
		{
			List<Connections> contacts = data.GetConnectionForCompany (companyId);
			if (contacts != null) {
				for (int i = 0; i < contacts.Count (); i++) {
					if ((contacts [i].ConnectionName.Equals (SSID))) {
						return result = true;
					} else {
						result = false;
					}
